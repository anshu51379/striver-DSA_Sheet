...
public:
    ...
    Node* helper(int pre[], int &cnt, int bound, int n){
        if(cnt==n or pre[cnt]>bound){
            return NULL;
        }
        Node* root=new Node();
        root->data=pre[cnt];
        cnt+=1;
        root->left=helper(pre, cnt, root->data, n);
        root->right=helper(pre, cnt, bound, n);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
        //code here
        int cnt=0;
        return helper(pre, cnt, INT_MAX, size);
    }
};
...
class Solution {
public:
    TreeNode* helper(vector<int> &preorder, int &cnt, int bound){
        if(cnt==preorder.size() or preorder[cnt]>bound){
            return NULL;
        }
        TreeNode* root=new TreeNode(preorder[cnt]);
        cnt+=1;
        root->left=helper(preorder, cnt, root->val);
        root->right=helper(preorder, cnt, bound);
        return root;
    }
    TreeNode* bstFromPreorder(vector<int>& preorder) {
        int cnt=0;
        return helper(preorder, cnt, INT_MAX);
    }
};
